...
const EditableInput = ({ initialValue, onSave, isEditing, setIsEditing, ...restProps }) => {
    ...
    return (
        <>
            <div className={`text-base leading-relaxed bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-400 flex w-full rounded items-center px-2 py-3 ${isEditing ? "border-white animate-pulse" : ""}`}>
                <div>Hey, </div>
                <input type="text"
                    value={value}
                    onChange={(e) => setValue(e.target.value)}
                    className="dark:bg-gray-800 outline-none px-2 w-full bg-white"
                    autoComplete='off'
                    disabled={!isEditing}
                    {...restProps}
                />
